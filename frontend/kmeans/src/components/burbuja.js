...
class Bubble2 extends Component {
  ...
  processImg() {
    ...
    if (this.props.iris_data.data) {
      let imgArray = JSON.parse(this.props.iris_data.data[0]);
      console.log(imgArray)
      for (var y = 0; y < height; y++) {
        for (var x = 0; x < width; x++) {
          var pos = (y * width + x) * 4; // position in buffer based on x and y
          buffer[pos] = imgArray[0][y][x][0]          // some R value [0, 255]
          buffer[pos + 1] = imgArray[0][y][x][1]               // some G value
          buffer[pos + 2] = imgArray[0][y][x][2]              // some B value
  ...
  render() {
    ...
    return (
      <div>
        <h2 onClick={this.processImg()}>Kmeans Graph</h2>
